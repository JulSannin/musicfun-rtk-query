...
[entities/artist](src/entities/artist/) устроен как `entities/tag` и намеренно на него похож, но три отличия ломают копипаст:

- `artists/search` **требует токен** (гостю 401), в отличие от `tags/search` с одним `API-KEY`. Отсюда и правило про фильтр треков выше: пикер видит только залогиненный.
- Ручки «отдай всех артистов» нет, только поиск по подстроке — значит `skip` на пустой строке (иначе 400) и `useDebounce`, как в `TagPicker`.
- Артиста можно **завести** прямо из поиска (`POST artists`), когда ничего не нашлось. Все проверки перед этим — на клиенте: длина 2–30 символов, точное совпадение уже в подсказках или в выбранных, — чтобы не ловить 400 и 409 на очевидном. Кнопка появляется только когда запрос уже отработал (`!isFetching`), иначе она мигала бы на каждой букве. 403 у этой ручки значит «уперся в лимит 100 артистов», 409 — «имя занято»; оба показывает `handleErrors`. Общий тег `Artists/LIST` на всех подстроках сбрасывается созданием, и новый артист появляется в подсказках, не дожидаясь новой буквы.

Мутацию создания зовёт **не пикер**, а тот, кто передал ему `onCreate`: [ArtistPicker](src/entities/artist/ui/ArtistPicker.tsx) лежит в `entities` и о мутациях не знает, а поставить рядом отдельную фичу нельзя — пикер рендерится внутри формы правки трека, а фича не может импортировать фичу. Без колбэка кнопка создания просто не рисуется: в фильтре треков заводить артиста незачем. Лимит в 5 артистов на трек — константа `TRACK_ARTISTS_MAX` в [trackForm.ts](src/entities/track/model/trackForm.ts), а не в слайсе артиста: лимит принадлежит треку, а у фильтра его нет вовсе (`max` необязателен).
