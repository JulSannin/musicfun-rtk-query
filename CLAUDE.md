...
### Панель подробностей трека

Подробности трека ([TrackDetails](src/widgets/track-details/ui/TrackDetails.tsx)) — не страница, а панель, и рендерит её [App.tsx](src/app/App.tsx) рядом с плеером. Три решения, которые стоит понимать вместе:

- **Почему не страница.** Список треков бесконечный, и уход на отдельный адрес терял бы позицию прокрутки: вернувшись, человек оказывался бы наверху. Панель держит список смонтированным.
- **Почему в `App`, а не на странице треков.** Плеер глобальный, и по клику на название в нём панель должна раскрываться поверх любой страницы, а не уводить на `/tracks`. Отсюда же следствие: «какой трек открыт» — общее действие, а не приватное дело страницы, поэтому оно живёт в [useTrackPanel](src/shared/lib/hooks/useTrackPanel.ts) в `shared`: `app` рисует панель, страница треков её открывает, и импортировать `app` страница не имеет права.
- **Состояние — в адресе** (`?track=<id>`, имя параметра в [paths.ts](src/shared/config/paths.ts)), а не в `useState`: ссылка на трек остаётся рабочей, «назад» закрывает панель.

Панель следует за плеером ([useFollowPlayingTrack.ts](src/app/model/useFollowPlayingTrack.ts)), и это место требует аккуратности:

- Ловится именно **смена** трека в плеере (сравнение с `useRef`), а не расхождение с панелью. Простое «показывать то, что играет» сделало бы невозможным открыть подробности трека, который сейчас не играет: панель мгновенно перекидывало бы обратно.
- Предыдущее значение запоминается **до** всех ранних выходов, включая случай закрытой панели, — иначе смена трека в фоне осталась бы незамеченной и открытую позже панель дёрнуло бы на давно проигранный трек.
- Адрес при этом **подменяется** (`replace`), а не пушится: автопереход по очереди иначе набил бы историю, и «назад» пришлось бы жать по разу на каждый проигранный трек.
- Закрытую панель смена трека не открывает.

Кнопка запуска внутри панели работает «в контексте того, что уже играет»: если трек лежит в очереди плеера, после него продолжится она же, иначе играет он один. Очередь страницы панель не видит намеренно — она живёт над страницами. Отдать `playTrack` очередь без этого id нельзя: он не найдёт индекс и молча ничего не сделает.
