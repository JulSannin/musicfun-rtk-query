...
- Но «энтити + фича» не обязано становиться виджетом: страница видит оба слоя, и этого достаточно. Виджет заводят, когда у склейки появляется своё состояние (режим редактирования в `PlaylistsList`) или второй потребитель. Список треков прожил так несколько итераций — [TrackItem](src/entities/track/ui/TrackItem.tsx) с [TrackReactions](src/features/track-reaction/ui/TrackReactions.tsx) склеивались прямо в `TracksPage`, — а виджетом [tracks-list](src/widgets/tracks-list/ui/TracksList.tsx) стал ровно в тот момент, когда ту же строку попросила вторая страница (библиотека). Это и есть рабочий признак: не «выглядит переиспользуемым», а «уже переиспользуется».
- Импорт между слайсами одного слоя запрещён (`fsd/forbidden-imports`), и это уже трижды повлияло на раскладку: карточка живёт внутри виджета [playlists-list](src/widgets/playlists-list/) (список её рендерит), а не отдельным слайсом `widgets/playlist-card`; общее для двух фич уезжает слоем ниже — поля формы в [PlaylistFormFields](src/entities/playlist/ui/PlaylistFormFields.tsx) вместе с лимитами из [playlistForm.ts](src/entities/playlist/model/playlistForm.ts) делят `playlist-create` и `playlist-update`; а [TagPicker](src/entities/tag/ui/TagPicker.tsx) не встроен внутрь `PlaylistFormFields`, хотя логически им там место, — `entities/playlist` не имеет права импортировать `entities/tag`. Композиция уезжает наверх, к тому, кто видит оба слайса: пикер ставится рядом с полями формы в [CreatePlaylistForm](src/features/playlist-create/ui/CreatePlaylistForm.tsx) и [UpdatePlaylistForm](src/features/playlist-update/ui/UpdatePlaylistForm.tsx), а также на [PlaylistsPage](src/pages/playlists/ui/PlaylistsPage.tsx). Это общее правило: если двум энтити нужно встретиться, они встречаются слоем выше, а не импортируют друг друга.
...
- Широкий тег списка (`Playlists/LIST`, `Track/LIST`) реакции намеренно **не** сбрасывают: он перезапрашивал бы всю страницу на каждый клик. Счётчики поэтому чинятся только руками, и если убрать патчи — цифра замрёт до следующего похода на сервер. Единственное исключение — узкие `Playlists/LIKED` и `Track/LIKED`, которые отдают только выдачи с `onlyLikedByMe`: там реакция меняет не число внутри строки, а **сам состав списка**, и патч бессилен — добавить строку, которой в ответе не было, ему неоткуда. Пока библиотека закрыта, сброс этих тегов ничего не запрашивает, а лишь помечает записи устаревшими; запрос уйдёт при её открытии. Цена в том, что клик по лайку **на самой** библиотеке (и на `/tracks` с включённым чекбоксом — это та же запись кеша) перезапрашивает все её загруженные страницы.
...
- Оба счётчика в `ReactionCounters` опциональны, а сама реакция допускает `null`, потому что выдачи отличаются: у плейлиста есть и `likesCount`, и `dislikesCount`; в списке треков только `likesCount`; в составе плейлиста нет ни одного, зато `currentUserReaction` может прийти `null`. Обе функции проверяют именно поле (`!== undefined`), а не тип реакции, поэтому одна и та же логика работает на всех трёх формах. То же видно в UI: [TrackReactions](src/features/track-reaction/ui/TrackReactions.tsx) принимает счётчики необязательными и без них рисует кнопки с одним `aria-pressed`, в отличие от [PlaylistReactions](src/features/playlist-reaction/ui/PlaylistReactions.tsx). Полные числа у трека есть только в `GET /playlists/tracks/{trackId}` — то есть в панели подробностей.
...
Рядом лежит [validateAudioFile.ts](src/shared/lib/validateAudioFile.ts) с тем же договором (`validateAudio(file, rules)` возвращает текст ошибки или `null`, правила — `TRACK_MP3_RULES`, 1 МБ), но **синхронный**: у аудио нет габаритов, ради которых пришлось бы декодировать файл. Проверяется расширение, а не `file.type`: у mp3 браузеры отдают то `audio/mpeg`, то `audio/mp3`, а иногда и пустую строку, тогда как спека говорит именно про расширение. `ALLOWED_AUDIO_EXTENSIONS` подставляется в `accept`, но `accept` — только подсказка браузеру, расширение он не гарантирует.
...
Второе исключение — [artistsApi.types.ts](src/entities/artist/api/artistsApi.types.ts), и оно асимметричное: у `POST /artists` запрос идёт в конверте (`CreateArtistRequestPayload`), а ответ приходит голым `ArtistRef`, без `data`/`attributes`. `GET /artists/search` тоже отдаёт готовый массив `{ id, name }` — ровно ту форму, в которой артисты лежат в атрибутах трека, поэтому ни `Get*Output`, ни `transformResponse` там нет вовсе (в отличие от `searchTags`, где конверт распаковывать приходится).
...
Часть состояния всё же уезжает в адрес, и в двух разных видах. Первый — query-параметр: поиск плейлистов лежит в `?search=` ([usePlaylists.ts](src/pages/playlists/model/usePlaylists.ts)), иначе уход на страницу плейлиста и «назад» стирали бы набранное. Имя параметра там локальная константа `SEARCH_PARAM`, а не запись в `paths.ts`: читает и пишет его только эта страница, в отличие от `TRACK_PARAM`, который нужен сразу двум слоям. Три детали приёма: в адрес уезжает уже **отложенное** значение, а не каждая набранная буква; пишется оно через `replace`, иначе «назад» пришлось бы жать по разу на символ; а номер страницы намеренно остался в `useState` — его правит проверка `page > pagesCount` прямо на рендере (см. ниже), и в адресе он этой правке только мешал бы.

Второй вид — id в самом пути (`/playlists/:playlistId`). Роут один на все плейлисты, поэтому при переходе с одного на другой компонент **не размонтируется** — меняется только аргумент запроса. Отсюда правило, повторённое в [usePlaylist.ts](src/pages/playlist/model/usePlaylist.ts), [useTrackDetails.ts](src/widgets/track-details/model/useTrackDetails.ts), [PlaylistTracks](src/widgets/playlist-tracks/ui/PlaylistTracks.tsx) и [PlayPlaylistButton](src/features/playlist-play/ui/PlayPlaylistButton.tsx): такие места читают `currentData`, а не `data`, — `data` держит ответ по прошлому id и покажет чужой плейлист. Тот же `currentData` и по той же причине стоит в формах редактирования.

Третий — открытая вкладка (`?tab=`), и она одна на две страницы, поэтому живёт в [useUrlTab](src/shared/lib/hooks/useUrlTab.ts): секция должна пережить уход на страницу плейлиста и «назад». Запись идёт через `replace` (иначе «назад» перелистывало бы секции по одной), новый набор строится из `prev`, а не с нуля (рядом может лежать `?track=` от открытой панели), значение из адреса чужое и сверяется со списком, а не приводится через `as`, и дефолт **дописывается** в адрес эффектом, а не подразумевается: иначе ссылка есть только у одной секции, а на вторую её не скопировать; туда же схлопывается мусор вида `?tab=zzz`. Третий параметр хука — «страница вот-вот уедет редиректом», и он не косметический: без него гость на `/profile` успел бы получить `?tab=` в адрес, который в ту же секунду сменится. Невыбранная секция при этом размонтирована, её подписка снята и запрос за ней не уходит — но данные не пропадают сразу, запись кеша живёт ещё минуту (`keepUnusedDataFor`), поэтому быстрый возврат мгновенный и без запроса. По той же причине запросы секций стоят со `skip` по вкладке (`!me?.userId || tab !== 'playlists'` в [useProfile.ts](src/pages/profile/model/useProfile.ts)). Само состояние секции от размонтирования не страдает, если хук зовут на уровне страницы: в [useLibrary.ts](src/pages/library/model/useLibrary.ts) оба списка запрашиваются всегда и гасятся только через `skip`, поэтому номер страницы у плейлистов переживает переключение вкладок, хотя разметка секции исчезала.
...
- Фильтр по тегам работает и у гостя: `tags/search` защищён только `API-KEY`. Bearer-токен требуют 27 операций из 41, но не поиск тегов — а вот `artists/search` требует — и это разводит две вещи, которые выглядят одной: сам список треков фильтр по `artistsIds` отдаёт кому угодно, но искать артистов гостю нечем. Поэтому пикер артистов прячется, как `onlyLikedByMe`, и уже выбранные при разлогине гасятся (`artistsFilter` в [useTracks.ts](src/pages/tracks/model/useTracks.ts)) — иначе остался бы включённый фильтр, который нечем снять.
...
- Теги идут мимо react-hook-form (`register` работает со строкой инпута, а тут массив объектов) и живут отдельным `useState`. В форме **правки** это `TagRef[] | null`, где `null` значит «не трогали» (см. `react-hooks/set-state-in-effect` выше), а в форме **создания** — обычный массив: серверного значения, к которому можно было бы откатиться, ещё не существует.
- Теги у нового плейлиста навешиваются **вторым запросом**: `POST /playlists` принимает только `title` и `description`, `tagIds` есть лишь у `PUT`. Поэтому [CreatePlaylistForm](src/features/playlist-create/ui/CreatePlaylistForm.tsx) после успешного создания сразу зовёт `updatePlaylist` — единственное место в проекте, где одна кнопка отправляет две мутации подряд. Форма чистится сразу после первой, не дожидаясь второй: плейлист уже существует, и повторная отправка завела бы второй такой же — по той же причине кнопка погашена на **всю** цепочку, а не только на первый запрос. Если второй запрос упадёт, плейлист останется без тегов: тост покажет `handleErrors`, а откатывать нечем, транзакции на две ручки в API нет. Оптимистичный патч `updatePlaylist` в этом сценарии не работает (проверено), хотя теги ему и передают: созданный плейлист в кеш списка ещё не приехал, и находить патчу нечего — теги появляются перезапросом. Цена всего действия — четыре запроса: POST, PUT и два перезапроса списка, потому что `Playlists/LIST` сбрасывают обе мутации.
...
- Слайс [playerSlice.ts](src/entities/player/model/playerSlice.ts) держит только сериализуемое: очередь снимков, индекс и `isPlaying`. Сам элемент — в `useRef` виджета (`serializableCheck` иначе заругается, и по делу), а прогресс — в локальном `useState`: `timeupdate` стреляет несколько раз в секунду, и dispatch на каждый тик перерисовывал бы всех подписчиков. В сторе живут только события, которые видит кнопка в списке. Прогресс при этом не одно число, а `{ trackId, time, loadedDuration }`, и числа чужого трека гасятся сравнением id прямо на рендере: сбрасывать их по событию нельзя — при `preload="none"` браузер не присылает `loadstart` до первого `play`, и на паузе после переключения показалась бы позиция прошлого трека. Заодно это избавляет от сеттера внутри `useEffect`, запрещённого правилом линтера, — тот же приём, что с тегами в форме редактирования.
- В очередь уходят **снимки** (`PlayerTrack`: id, название, имена артистов, url, длительность, обложка), а не ссылки на кеш. Причин три: `refetchOnFocus` заменяет объект трека при возврате на вкладку; запись `fetchTracks` лежит под своим набором аргументов и после смены фильтров может исчезнуть; а в списке треков плейлиста форма атрибутов вообще другая. Собирает снимки [toPlayerTracks.ts](src/entities/player/model/toPlayerTracks.ts). Он лежит в `entities/player`, хотя разбирает ответ со списком треков: его параметр описан **структурно**, а не через типы из `entities/track` — импортировать соседний слайс энтити нельзя. Благодаря этому одна и та же функция принимает и общий список треков, и состав плейлиста (формы атрибутов у них разные: в составе нет `likesCount`, `user` и `isPublished`), и зовут её отовсюду, где нужна очередь: страница треков, библиотека, состав плейлиста, кнопка запуска плейлиста и подстановка очереди при старте приложения.
- Плеер сам добывает себе первую очередь ([usePrimePlayer.ts](src/widgets/player/model/usePrimePlayer.ts) + `primeQueue` в слайсе), а не ждёт захода на `/tracks`: иначе полоса внизу встречала бы человека погашенными кнопками. Дешёвым это держат две вещи. Аргументы запроса берутся из [trackListDefaults.ts](src/entities/track/model/trackListDefaults.ts), и теми же константами стартует [useTracks.ts](src/pages/tracks/model/useTracks.ts): ключ кеша RTK Query — это аргумент, и стоит значениям разойтись, как за тем же самым списком уйдёт второй запрос. А подписка снимается по `skip: Boolean(currentTrackId)` — запрос уходит ровно один раз за сессию; фоновые перезапросы у неё выключены, как и у самого списка треков. Сам `primeQueue` только подставляет очередь и ставит индекс в 0, не начиная воспроизведение, и молча ничего не делает, если трек уже выбран: подменять человеку то, что он поставил на паузу, нельзя.
...
Мелочи, на которых легко споткнуться: трек без mp3 (`attachments` пуст) в очередь не берётся вообще, иначе автопереход встанет на нём намертво; `audio.play()` реджектится `AbortError`, если `src` сменился до старта (быстрые клики по разным трекам), поэтому `.catch` обязателен; у `<audio>` стоит `preload="none"` — без него браузер лез бы за метаданными mp3 при каждом запуске приложения, хотя длительность и так известна из ответа сервера, а битую ссылку честнее показывать по нажатию play; по той же причине присваивание `audio.currentTime` завёрнуто в `try` — до первого `play` метаданных нет, перемотка может бросить, а обработчик падать не должен. И `audio.duration` берётся только через `Number.isFinite`: у mp3 без длительности в заголовках браузер отдаёт `Infinity`, ползунок с таким `max` перестаёт двигаться, а `formatDuration(Infinity)` печатает `Infinity:NaN:NaN` (проверено, `NaN` функция гасит, `Infinity` — нет). Обработчики `mediaSession` (кнопки на гарнитуре и в шторке ОС) обязательно снимаются в cleanup — иначе останутся висеть от прошлого трека.
...
Страница плейлиста (`/playlists/:playlistId`) собирает шапку из уже готовых кусков, а состав отдаёт виджету [PlaylistTracks](src/widgets/playlist-tracks/ui/PlaylistTracks.tsx) — он ходит за данными сам, страница передаёт только id и посчитанное владение.

Четыре эндпоинта состава живут в [tracksApi](src/entities/track/api/tracksApi.ts), а не в `playlistsApi`, по жёсткой причине: ответ — это треки, а `entities/playlist` не имеет права импортировать типы из `entities/track`. Отсюда следствие, за которым надо следить: типы тегов `Playlists` и `Playlist` объявлены **в двух местах** (`enhanceEndpoints` обоих слайсов). Дубль безвреден, типы тегов глобальны для `baseApi`, но опечатка в имени тихо сломает инвалидацию — а сбрасывать эти теги обязательно, иначе после добавления трека в карточке останутся старые `tracksCount` и длительность.

- `GET /playlists/{playlistId}/tracks` не имеет ни одного query-параметра: пагинации нет вовсе, в плейлист влезает максимум 10 треков (одиннадцатый — 403 с текстом про лимит, тем же кодом, что и «чужой плейлист»).
- Форма атрибутов здесь своя: есть `order` и `updatedAt`, но **нет ни одного счётчика реакций**, нет `user` и `isPublished`, а `currentUserReaction` может прийти `null`. Кнопки лайка и дизлайка в составе при этом есть — без чисел, с одним состоянием: реакция у трека одна на всё приложение, и действие полноценное. Чтобы это стало возможно, счётчики в `ReactionCounters` сделаны необязательными (см. выше), а `null` нормализуется в `CurrentUserReaction.None` на месте отрисовки. Патч кеша реакции проходит и по записям `fetchPlaylistTracks` — иначе кнопка в составе замирала бы после клика.
- Порядок нормализуется по `order` **один раз, в `transformResponse`**, а не при отрисовке. Это не стилистика: на `reorder` сервер отвечает пустым телом и новых номеров не присылает, поэтому сортировка в компоненте отменяла бы оптимистичный патч, и перестановка выглядела бы неработающей. После входа в кеш массивом распоряжается только патч.
- Перестановка (`PUT .../reorder`) принимает не позицию, а **id соседа**, после которого встать (`null` — в начало). Отсюда арифметика в [PlaylistTrackActions](src/features/playlist-track-actions/ui/PlaylistTrackActions.tsx): вверх — `orderedIds[index - 2] ?? null`, вниз — `orderedIds[index + 1]`. Собственный id подставлять нельзя, на него сервер отвечает 400 «cannot place a track after itself».
- У перестановки `invalidatesTags` нет намеренно, только оптимистичный патч: сброс заставлял бы список прыгать на каждое нажатие.
- Добавление трека ([AddTrackToPlaylist](src/features/playlist-track-add/ui/AddTrackToPlaylist.tsx)) — это поиск по общему списку треков, из выдачи которого уже добавленные вычёркиваются **на клиенте**: ручки «найди то, чего ещё нет в плейлисте» в API нет. Запрос дебаунсится и уходит со `skip` на пустой строке, как в `TagPicker`, и берётся только первая страница `infiniteQuery` — пикер это не список, докручивать в нём нечего. Лимит в 10 треков проверяется ещё раз в обработчике клика, а не только через `disabled` у инпута: подсказки остаются в разметке и кликабельны.
- Запуск плейлиста ([PlayPlaylistButton](src/features/playlist-play/ui/PlayPlaylistButton.tsx)) подписан на тот же `fetchPlaylistTracks` с тем же аргументом, что и виджет состава, — попадает в ту же запись кеша, второго запроса нет. Кнопка сама решает, показываться ли: пустой плейлист и плейлист из одних треков без mp3 играть нечем, а мёртвая кнопка хуже её отсутствия. Если этот плейлист уже в плеере, клик продолжает с текущего трека, а не перескакивает на первый.

### Свои треки

Управление собственными треками живёт на `/profile`, а не в общем списке `/tracks`: там треки в основном чужие. Страница отдаёт список виджету [MyTracks](src/widgets/my-tracks/ui/MyTracks.tsx), и вот почему это виджет, хотя список треков на `/tracks` виджетом не стал: тут склейка энтити с четырьмя фичами (публикация, правка, обложка, удаление) плюс своё состояние — какой трек сейчас редактируется. Владение внутри не проверяется: [useMyTracks](src/widgets/my-tracks/model/useMyTracks.ts) и так просит выдачу по своему `userId` вместе с `includeDrafts` (без флага человек не увидел бы собственных черновиков) и ждёт `me` через `skip`, склеивая `isMeLoading || isLoading` — иначе список успел бы моргнуть пустым. Сортировка берётся теми же константами из [trackListDefaults.ts](src/entities/track/model/trackListDefaults.ts), что у страницы треков и у плеера.

Жизненный цикл трека размазан по четырём ручкам, и у каждой своя оговорка:

- **Загрузка** (`POST playlists/tracks/upload`) всегда создаёт **новый** трек: догрузить mp3 к существующему API не умеет вовсе, поэтому форма прячется за кнопкой, а не висит над списком. Тело — `FormData` с полями `title` и `file`; имена из свагера, и у обложки того же трека поле называется `cover` — копипаст не сработает. Трек появляется **черновиком**.
- **Публикация** (`POST .../actions/publish`) необратима: снять трек с публикации в API нечем. Поэтому кнопка есть только у черновика — у опубликованного она была бы мёртвой.
- **Правка** (`PUT playlists/tracks/{id}`) заменяет трек целиком, частичного обновления у ручки нет: форма обязана отправлять и то, чего не трогали, включая `tagIds` и `artistsIds` (пустой массив означает «снять всех»).
- **Удаление** (`DELETE playlists/tracks/{id}`) убивает трек насовсем — не путать с `removeTrackFromPlaylist`, та оставляет трек жив. Отсюда нативный `confirm()`, как и у плейлиста: ответ нужен до запроса.

Кеш треков чинится в основном патчами, и это осознанно: у `fetchTracks` ровно один тег `Track/LIST`, а его сброс перезапрашивает **все** подгруженные страницы **всех** вариантов списка. Тег сбрасывают только две мутации, обе редкие: `uploadTrack` (куда сервер поставит новый трек в текущей сортировке, клиент не знает, а вставлять его наугад в начало нельзя — те же записи кеша держат и общий список, где чужому черновику не место) и `updateTrack` (имена артистов лежат в `included` страницы, а не в атрибутах трека, и точечная правка там хрупкая). Остальные правят записи руками, для чего в [tracksApi.ts](src/entities/track/api/tracksApi.ts) заведены два помощника: `trackListPatches` — все варианты `fetchTracks` со всеми их страницами плюс все закешированные составы плейлистов (параметр `mutate` описан структурно и намеренно узко: общее у двух выдач только `title` и `images`), и `trackDetailsPatch` — карточка `fetchTrack`, где аргумент обязан совпасть с тем, чем зовёт хук панели, иначе ключ кеша другой и патч уйдёт в пустоту. Два действия из общего помощника выпадают: `publishTrack` патчит только `fetchTracks` (поля `isPublished` в составе плейлиста нет вовсе) и сбрасывает точечный `Track/{id}` — дату публикации проставляет сервер, а видна она в карточке; `deleteTrack` инвалидирует `Playlists/LIST`, потому что у карточек меняются `tracksCount` и длительность, а в каких плейлистах лежал трек, клиент не знает. Обложка трека патчит кеш **после** ответа, а не оптимистично: превью режет сервер, и урлы вариантов угадать нечем.

Формы трека повторяют раскладку плейлиста (состояние `useForm` в фиче, поля с правилами — в [TrackFormFields](src/entities/track/ui/TrackFormFields.tsx) из `entities`), но у них четыре своих места:

- Название вынесено отдельным `TrackTitleField`: у загрузки поля на нём и заканчиваются, ручка `upload` принимает ровно `title` и `file`, ни текста, ни даты релиза в ней нет.
- Файл читается из `ref` инпута на отправке, а не хранится в `useState` по `onChange`: после неудачной проверки выбор **того же** файла заново не вызвал бы `onChange` (значение инпута не изменилось), и кнопка выглядела бы сломанной. По той же причине `event.target.value` сбрасывается сразу в [TrackCoverActions](src/features/track-cover/ui/TrackCoverActions.tsx). А раз `onSubmit` читает ref, `handleSubmit` зовётся внутри обработчика (`onSubmit={(e) => void handleSubmit(onSubmit)(e)}`), а не прямо в атрибуте: правило `react-hooks/refs` считает вызов в атрибуте чтением ref во время отрисовки.
- В [UpdateTrackForm](src/features/track-update/ui/UpdateTrackForm.tsx) `reset` по ответу сервера защищён отметкой `filledForRef`: рядом стоят действия над обложкой и реакции, их патчи меняют ту же запись `fetchTrack`, `attributes` приезжает новым объектом — без отметки эффект срабатывал бы снова и затирал набранное. Форму заполняют именно отдельным запросом `fetchTrack`, а не данными списка: `lyrics`, `releaseDate`, тегов и артистов в выдаче списка нет.
- Даты: `input[type=date]` работает с `yyyy-MM-dd`, сервер отдаёт и ждёт ISO 8601. Перекладывает это форма — `slice(0, 10)` при заполнении, `new Date(value).toISOString()` при отправке; строка без времени разбирается как UTC, поэтому день не съезжает.

### Библиотека

`/library` — понравившееся: две вкладки, треки и плейлисты. Своих ручек у неё нет вовсе, это `onlyLikedByMe` у обоих списков, то есть ровно те же чекбоксы, что на `/tracks` и `/playlists`, вынесенные отдельной страницей. Чекбоксы при этом остались: там фильтр комбинируется с поиском и тегами («понравившееся среди рока»), а библиотека — быстрый вход без фильтров.

- Аргументы запроса треков берутся из [trackListDefaults.ts](src/entities/track/model/trackListDefaults.ts), и это не косметика: набор совпадает с тем, что шлёт `/tracks` с включённым чекбоксом, а ключ кеша RTK Query — это аргумент. Совпал — переход между страницами проходит вообще без запроса; разошёлся — уедет второй запрос за тем же списком.
- Обе выдачи требуют токена, поэтому страница целиком под авторизацией: `isUnauthorized` считается как на профиле (`!isMeLoading && !me`) и уводит `<Navigate replace />`. По той же причине пункт `Library` в шапке показывается только залогиненному — ссылка, которая всегда редиректит, хуже её отсутствия.
- Состав этих двух списков меняет сама реакция, поэтому они единственные, кого реакция инвалидирует — узкими тегами `Track/LIKED` и `Playlists/LIKED` (см. «Реакции и патч кеша»). Без них лайк, поставленный на `/tracks`, в библиотеку не приезжал бы вовсе: патч правит счётчики внутри уже полученных строк и не может добавить строку, которой в ответе не было, а запись кеша живёт ещё минуту после ухода со страницы — то есть возврат в библиотеку в пределах этой минуты показывал бы старый список без запроса.
- Счётчика «всего понравившихся треков» не будет: в курсорном режиме сервер не считает `totalCount`. У плейлистов он есть, поэтому там обычная пагинация номерами — две модели пагинации на одной странице, как и в остальном приложении.

### Артисты

[entities/artist](src/entities/artist/) и [entities/tag](src/entities/tag/) устроены одинаково, и это намеренно. У обоих нет ручки «отдай всё», только поиск по подстроке — отсюда `skip` на пустой строке (иначе 400) и `useDebounce`. Оба умеют **заводить** новую запись прямо из поиска, когда ничего не нашлось, с одинаковыми правилами: 2–30 символов, 403 значит «уперся в лимит 100 на пользователя», 409 — «имя занято». Всё, что видно на клиенте, проверяется до запроса (длина, точное совпадение среди найденных и уже выбранных), чтобы не ловить эти ошибки на очевидном, а кнопка создания появляется только когда запрос уже отработал (`!isFetching`) — иначе она мигала бы на каждой букве. Общий тег (`Tags/LIST`, `Artists/LIST`) сбрасывается созданием, и новая запись попадает в подсказки, не дожидаясь новой буквы.

Два отличия всё же ломают копипаст:

- `artists/search` **требует токен** (гостю 401), в отличие от `tags/search` с одним `API-KEY`. Отсюда и правило про фильтр треков выше: пикер артистов видит только залогиненный.
- Ответы создания разной формы. `POST /artists` отдаёт голый `ArtistRef`, а `POST /tags` — конверт `{ data: { id, attributes } }`, как и поиск тегов, поэтому у `createTag` есть `transformResponse`, а у `createArtist` его нет вовсе. Наружу обе мутации отдают одинаковый `TagRef`/`ArtistRef`, и вызывающий код кладёт созданное в выбранные как есть.

Мутацию создания зовёт **не пикер**, а тот, кто передал ему `onCreate`: [ArtistPicker](src/entities/artist/ui/ArtistPicker.tsx) и [TagPicker](src/entities/tag/ui/TagPicker.tsx) лежат в `entities` и о мутациях не знают, а поставить рядом отдельную фичу нельзя — пикеры рендерятся внутри форм правки, а фича не может импортировать фичу. Без колбэка кнопка создания просто не рисуется: в фильтрах списков заводить тег или артиста незачем, и передают `onCreate` только формы: создание и правка плейлиста, правка трека. Лимит в 5 артистов на трек — константа `TRACK_ARTISTS_MAX` в [trackForm.ts](src/entities/track/model/trackForm.ts), а не в слайсе артиста: лимит принадлежит треку, а у фильтра его нет вовсе (`max` необязателен).

Сам пикер, как и `TagPicker`, встречается с полями формы **слоем выше** — в [UpdateTrackForm](src/features/track-update/ui/UpdateTrackForm.tsx) и на [TracksPage](src/pages/tracks/ui/TracksPage.tsx): `entities/track` не имеет права импортировать ни `entities/tag`, ни `entities/artist`. Выбранные артисты, как и теги, идут мимо react-hook-form отдельным `useState<ArtistRef[] | null>`, где `null` значит «не трогали».

### Списки и владение

[PlaylistsList](src/widgets/playlists-list/ui/PlaylistsList.tsx) — один виджет на три страницы: `/playlists` (поиск + пагинация), `/profile` (только свои) и `/library` (только понравившиеся). Различия задаются пропсами (`isFetching`, `emptyText`), а не копией разметки. Рядом с ним теперь стоит [TracksList](src/widgets/tracks-list/ui/TracksList.tsx), устроенный так же: разметка и склейка с фичами внутри, а запрос со всеми фильтрами — снаружи, в хуке страницы.
...
Третий случай — экран, который целиком зависит от одного объекта: страница плейлиста, панель подробностей, состав плейлиста. Там загрузка считается как `!currentData && !isError` («нечего показать, и это не ошибка»), а не по флагам хука. Ни `isLoading`, ни `isFetching` не годятся: до старта запроса RTK Query отдаёт `isFetching === undefined` и `isLoading === false`, и на первом же рендере экран мигнул бы ошибкой. Опора на `currentData` заодно ловит переход с одного id на другой, а фоновый перезапрос по фокусу экран не гасит — данные по текущему id уже есть. По тому же признаку пустой список отличается от ещё не приехавшего, иначе «This playlist is empty» мигало бы на каждой загрузке.
...
Вниз передаются только те поля, которые компонент рисует, а не вся сущность: `PlaylistInfo` берёт `title`, `authorName`, `tracksCount`, `duration` и готовые имена тегов (`tagNames`, а не сами `TagRef`), `PlaylistCover` — одни `images`. По той же причине `TrackItem` получает `artistNames` готовым массивом: разбор `included` из ответа JSON API живёт в [toTrackListItems.ts](src/entities/track/model/toTrackListItems.ts) — связь «id артиста → имя» одинаковая у всех трёх списков треков (общий, свои, понравившиеся), и копий у неё быть не должно. Целый объект принимает только тот компонент, который стоит на границе списка ([PlaylistCard](src/widgets/playlists-list/ui/PlaylistCard.tsx), [TrackItem](src/entities/track/ui/TrackItem.tsx)). Компоненты из `shared/ui` про запросы не знают вообще — принимают значения и отдают колбэки.
...
Два компонента показывают, как это правило живёт, когда у компонента появляется второй потребитель. У [PlaylistInfo](src/entities/playlist/ui/PlaylistInfo.tsx) два необязательных пропа: `description` (в ответе списка этого поля нет вовсе, оно есть только у одного плейлиста) и `to` — **готовый адрес**, а не id: путь собирает через `playlistPath` тот, кто знает про роутер, а без `to` название остаётся обычным текстом, как на самой странице плейлиста. В [PlaylistCard](src/widgets/playlists-list/ui/PlaylistCard.tsx) эта единственная `<a>` растягивается на всю карточку псевдоэлементом (`.card a::after` с `inset: 0`), а всё кликабельное поднимается над ней через `z-index`. Обёртка-ссылка вокруг карточки не годится: внутри кнопки, а `<a>` с кнопками внутри — невалидная разметка с ломающимися кликами. [TrackCover](src/entities/track/ui/TrackCover.tsx) принимает `size`: от него зависит, какой файл просить у сервера (`thumbnail` в строке списка, `original` в развёрнутой карточке), но любой доступный вариант лучше пустоты — набор размеров у разных треков разный. Заглушки для трека в проекте нет (в отличие от плейлиста), поэтому пустое место рисуется руками, иначе строки списка разъезжаются по высоте.
...
- [Tabs](src/shared/ui/Tabs/Tabs.tsx) — переключатель секций, дженерик по `T extends string` по той же причине, что у `Select`. Про адрес не знает вовсе: отдаёт наверх значение, а кто и куда его пишет — дело страницы. Активная вкладка помечена `aria-pressed` и **не** `disabled` — как текущая страница в пагинации.
- [useUrlTab](src/shared/lib/hooks/useUrlTab.ts) — та самая запись вкладки в `?tab=` со всеми оговорками из «Состояния страниц».
...
Пути только из объекта `paths` в [src/shared/config/paths.ts](src/shared/config/paths.ts), строками в компонентах не писать. Все роуты объявлены в [Routing.tsx](src/app/routing/Routing.tsx). Путь с параметром наружу отдаётся не строкой, а функцией `playlistPath(id)` из того же файла: подстановка id живёт рядом с шаблоном роута, а не расползается по карточкам. Там же лежит `TRACK_PARAM` — своего адреса у трека нет, подробности раскрываются панелью через query-параметр.
