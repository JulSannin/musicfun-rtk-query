        ...
        <div>
            ...
            {canCreate && (
                <button
                    type="button"
                    onClick={() => {
                        // поле чистим только после успеха: на 409 («имя занято»)
                        // и 403 («лимит 100») набранное должно остаться
                        // на экране, иначе его придётся набирать заново —
                        // то же правило, что с reset() у форм.
                        // Вызывающий возвращает промис мутации и ошибку
                        // не глотает, тост по ней уже показал handleErrors
                        void Promise.resolve(onCreate?.(query))
                            .then(() => setSearch(''))
                            .catch(() => {});
                    }}
                    disabled={isCreating}
