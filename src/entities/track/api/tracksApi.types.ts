...
export type FetchTracksArgs = {
    ...
    tagsIds?: string[];
    // сам фильтр сервер отдаёт кому угодно, но искать артистов гостю нечем:
    // artists/search, в отличие от поиска тегов, требует bearer-токен —
    // поэтому пикер в UI прячут от неавторизованного
    artistsIds?: string[];
    userId?: string;
