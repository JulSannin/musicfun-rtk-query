    ...
    .injectEndpoints({
        endpoints: (build) => ({
            ...
            >({
                ...
                // Track/LIST — тег всех вариантов списка разом, и сбрасывают
                // его только загрузка и правка трека. Остальные мутации чинят
                // кеш патчами именно поэтому: сброс перезапрашивает ВСЕ
                // подгруженные страницы всех вариантов списка, и на каждый
                // клик по лайку это было бы недопустимо дорого.
                //
                // Track/LIKED получают только выдачи «понравившееся» — у них
                // реакция меняет не счётчик внутри строки, а сам состав списка,
                // и патчем это не чинится: откуда бы взяться строке, которой
                // в ответе не было. Отдельный id вместо LIST держит цену низкой:
                // сбрасываются ровно эти записи, а не весь список треков
            ...
            fetchTrack: build.query<GetTrackDetailsOutput, FetchTrackArgs>({
                ...
                // точечный тег: правка одного трека не должна сбрасывать
                // карточки остальных. Сбрасывает его только публикация —
                // дату публикации проставляет сервер, и другого способа
                // её узнать нет; правка и обложка кладут в карточку ответ
                // мутации патчем, а реакции чинят кеш руками
            ...
            >({
                ...
                // ради тех же данных незачем. Списку сброс тоже не нужен:
                // Track/LIST перезапросил бы все загруженные страницы каждого
                // варианта выдачи — дороже самой загрузки файла, а картинки
                // туда уже положил патч выше
            }),
            ...
            deleteTrack: build.mutation<void, { trackId: string }>({
                ...
                async onQueryStarted({ trackId }, lifecycleApi) {
                    ...
                    try {
                        ...
                        // строку убираем руками: сброс Track/LIST перезапросил
                        // бы все подгруженные страницы каждого варианта выдачи
                        // ради одной исчезнувшей строки — дороже самого удаления
            ...
            >({
                ...
                // Track/LIST не сбрасываем намеренно: он перезапросил бы
                // карточку и все загруженные страницы каждого списка на каждый
                // клик по кнопке. Обычные списки чинят патчи выше.
                //
                // А вот выдачу «только понравившееся» патч починить не может:
                // лайк добавляет в неё строку, которой в ответе не было, снятый
                // лайк — убирает. Поэтому сбрасываем узкий Track/LIKED: пока
                // библиотека закрыта, записи просто помечаются устаревшими
                // и перезапросятся при следующем открытии, а не сейчас
                invalidatesTags: [{ type: 'Track', id: 'LIKED' }],
            }),
        }),
    });
